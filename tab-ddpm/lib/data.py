...
def normalize(
    ...
    if pipeline_dict_path is not None:
        ...
        if pipeline_dict["num_transform"] is not None:
            print("Loading numerical preprocessing pipeline from", pipeline_dict_path)
            normalizer = pipeline_dict["num_transform"]
...
def cat_encode(
    ...
    if encoding is None:
        if pipeline_dict_path is not None:
            pipeline_dict = joblib.load(pipeline_dict_path)
            if pipeline_dict["cat_transform"] is not None:
                print(
                    "Loading categorical preprocessing pipeline from",
                    pipeline_dict_path,
                )
                encoder = pipeline_dict["cat_transform"]
                unknown_value = encoder[0].unknown_value
        else:
