...
parser = argparse.ArgumentParser(description="Syn-Boost with CatBoost tuning")

parser.add_argument("--dsname", type=str, default="insurance")

# parser.add_argument("--metric", type=str, default="rmse")
# whether to use alternative metric for evaluation: "r2" to replace "rmse" and "f1" to replace "acc"

parser.add_argument("--maxrho", type=int, default=30, help="maximum fake-to-real ratio")
parser.add_argument("--nratios", type=int, default=30, help="number of fake-to-real ratios")
parser.add_argument("--ntrials", type=int, default=100, help="number of trials for tuning hyperparameters of CatBoost models trained on synthetic data")
parser.add_argument("--device", type=str, default="cuda:0")
parser.add_argument("--altmetric", action = "store_true", help="whether to use alternative metric for evaluation: 'r2' to replace 'rmse' and 'f1' to replace 'acc'")

# default metrics associated with each dataset
map_dict = {
    "adult": [0, "acc"],
    "insurance": [1, "rmse"],
    "abalone": [2, "rmse"],
...
SEED = 0
TABDDPM_FOLDER = os.path.join(REPO_DIR, "tab-ddpm")
INFERENCE_FOLDER = os.path.join(REPO_DIR, "tab-ddpm/synpred")
...
EVALUATION_METRIC = map_dict[DS_NAME][1] if not args.altmetric else "r2" if map_dict[DS_NAME][1] == "rmse" else "f1"
...
MAX_FAKE_TO_REAL_RATIO = args.maxrho
NUM_RATIOS = args.nratios
