...
class GaussianMultinomialDiffusion(torch.nn.Module):
    ...
    def gaussian_p_sample(
        ...
        # this is a part where we actually sample, TO BE MODIFIED for zero noising (add additional argument)
        sample = out["mean"] + (
    ...
    def log_sample_categorical(self, logits, uniforms=None):
        full_sample = []
        for i in range(len(self.num_classes)):
            one_class_logits = logits[:, self.slices_for_classes[i]]
            if uniforms is None or uniforms.numel() == 0:
                uniform = torch.rand_like(one_class_logits)
            else:
                # use provided uniforms if provided
                uniform = uniforms[:, self.slices_for_classes[i]]
    ...
    @torch.no_grad()
    # perturb_dict has keys: "tau", "source_dist", "noise_dist"
    def sample_ddim(
        self, num_samples, y_dist, perturb_dict=None, dataset=None, parent_dir=None
    ):
        b = num_samples
        ...
        # z_norm is probably x_T (pure white Gaussian noise at the end of forward process, and beginning of reverse process)
        ...
        for i in reversed(range(0, self.num_timesteps)):
            ...
            if has_cat:
                ...
                # (cont.) it will be the input to the multinomial part of the MLP reverse process, the output of the MLP is still logits
                log_z = self.multinomial_ddim_step(
                    model_out_cat, log_z, t, out_dict
                )  # get logits first, then logits to log_z (log of clamped one-hot encoding)
    ...
    @torch.no_grad()
    # perturb_dict has keys: "tau"
    # parent_dir is the one specified by the TOML configuration file
    def sample(
        self,
        num_samples,
        y_dist,
    ...
    def sample_all(
        ...
        if perturb_dict is not None:
            print("Sample using PASS.")

        if deterministic:
            print("Sample deterministically by setting added noise to 0.0.")
