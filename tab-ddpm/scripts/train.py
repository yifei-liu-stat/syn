...
def train(
    ...
    """
    Some arguments:
    - parent_dir: the parent directory to save the model checkpoint and the preprocessing pipelines dictionary
    - model_path: path to the model checkpoint. Set as None if training from scratch.
    ...
    # when provided, load model checkpoint if provided
    ...
    pipeline_dict = {
        "num_transform": dataset.num_transform,
        "cat_transform": dataset.cat_transform,
        "category_sizes": K,
    }
    joblib.dump(pipeline_dict, os.path.join(parent_dir, "pipeline_dict.joblib"))
