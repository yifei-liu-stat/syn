...
# WARNING: I add this by myself, will only affect MLP tuning for now, discard this for all other cases
...
def suggest_mlp_params(trial):
    ...
    params["weight_decay"] = _suggest_optional(
        trial, "loguniform", "weight_decay", 1e-6, 1e-2
    )
...
def objective(trial):
    ...
    if args.tune_type == "cv":
        ...
        if args.metric == "":
            ...
            # acc, f1, r2
            score = metrics_report.get_metric("val", args.metric)
        else:
            # rmse
            score = -metrics_report.get_metric("val", args.metric)

    trial.set_user_attr("metrics_report", metrics_report._res)
...
lib.dump_json(best_params, best_params_path)


# also save the metrics report: all splits and all metrics
best_metrics = study.best_trial.user_attrs["metrics_report"]
best_metrics_path = (
    f"tuned_models/{args.model}/{args.ds_name}_{args.tune_type}_metrics.json"
)
lib.dump_json(best_metrics, best_metrics_path)
