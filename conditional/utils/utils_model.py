...
class MLPDiffusionContinuous(nn.Module):
    """
    MLP backbone for the tabular diffusion model with time embedding.
    - d_in: input dimension, the MLP will output a vector of this dimension as well
    - hidden_dims: list of hidden layer dimensions, e.g. [512, 256, 256, 256, 128]. Check the configuration file under the same directory.
    - dim_t: the dimension of the time embedding

    NOTE: this class only support tabular data with continuous columns
    """

    def __init__(self, d_in, hidden_dims, dim_t=128):
        super().__init__()
        self.dim_t = dim_t

        self.mlp = MLP.make_baseline(
            d_in=dim_t, d_layers=hidden_dims, dropout=0.0, d_out=d_in
        )
